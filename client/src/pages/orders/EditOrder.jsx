...
const EditOrder = () => {
  ...
  useEffect(() => {
    axios.get(backendUrl + `/api/orders/${id}`)
      .then(({ data }) => {
        if (data.success) {
          const o = data.order
          setVehicle(o.vehicle || { make: '', model: '', year: '', engine: '' })
          setParts(o.parts.map(p => ({ ...p, imageMode: p.imageUrl ? 'url' : 'url' })))
          setNotes(o.notes || '')
        } else {
      ...
      .catch(() => { toast.error('Захиалга ачаалахад алдаа гарлаа'); navigate('/orders') })
  ...
  const handleImageFile = async (index, file) => {
    ...
    try {
      ...
      if (data.success) updatePart(index, 'imageUrl', data.url)
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      const { data } = await axios.patch(backendUrl + `/api/orders/${id}`, { parts: validParts, vehicle, notes })
      if (data.success) {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
      <div className="pt-24 text-center text-gray-400">Уншиж байна...</div>
    </div>
  )
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-24 px-4 sm:px-8 max-w-3xl mx-auto pb-10">
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => navigate(`/orders/${id}`)} className="text-gray-400 hover:text-gray-600 text-sm">
            ← Буцах
          </button>
          <h1 className="text-2xl font-bold text-gray-800">Захиалга засах</h1>
        </div>
              ...
              {parts.map((part, index) => (
                <div key={index} className="border border-gray-100 rounded-xl p-4 bg-gray-50">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {parts.length > 1 && (
                      <button type="button" onClick={() => removePart(index)} className="text-red-400 hover:text-red-600 text-sm">
                        Устгах
                      </button>
                    )}
                  </div>
